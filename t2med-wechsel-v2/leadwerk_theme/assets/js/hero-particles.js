/**
 * Hero circuit traces — technical network / PCB schematic.
 */
(function () {
    'use strict';
    ...
    function buildLayout() {
        ...
        pads.push({ x: w * 0.18, y: h * 0.08, hub: false });
        pads.push({ x: w * 0.26, y: h * 0.28, hub: false });
        pads.push({ x: w * 0.14, y: h * 0.36, via: true });

        // —— Right circuit cluster (runs to bottom) ——
        traces.push({
            pts: [
                { x: w + 10, y: h * 0.14 },
                { x: w * 0.82, y: h * 0.14 },
